...
const BtnAction = ({ children, path, onClick, type = 'button', ...props }: Props) => {
  return (
    <>
      {
        !path
          ? <button {...props} type={type} onClick={onClick} className=" relative z-10 flex items-center justify-center border-none bg-[#11E0F8] rounded-md px-10 py-5px text-white h-[36px]  mt-10 text-center font-bold text-lg hover:opacity-60 transition-all duration-200">{children}</button>
          : <Link
            {...props}
            to={path}
