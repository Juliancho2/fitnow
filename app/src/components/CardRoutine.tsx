...
import { deleteExerciseFromRoutine } from '../thunk'
import { activeEditExerciseModal, setExerciseSidebar } from '../slice/dataExerciseSlice'

type Props = {
  exerciseData: DataFromApiExercise,
  dayActive: string,
  setOpenSideBar: React.Dispatch<SetStateAction<boolean>>,
  dragStart: (evt: any, item: any) => void
}


const CardRoutine = ({ exerciseData, dayActive, setOpenSideBar, dragStart }: Props) => {
  ...
  return (
    <>
      ...
      <div draggable onDragStart={(e) => dragStart(e, exerciseData)} className="cursor-move block max-w-xs p-4 shadow-sm shadow-indigo-100  rounded-md bg-white relative border">
